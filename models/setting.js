...
class Setting {
  constructor () {
    /** 默认设置 */
    this.defPath = `${_path}/plugins/tea-plugin/defSet/`
    this.defSet = {}

    /** 用户设置 */
    this.configPath = `${_path}/plugins/tea-plugin/config/`
    this.config = {}

    this.dataPath = `${_path}/plugins/tea-plugin/data/`
    this.data = {}
  ...
  getFilePath (app, type) {
    if (type === 'defSet') return `${this.defPath}${app}.yaml`
      ...
      try {
        ...
        logger.error(`茶插件缺失默认文件[${app}]${error}`)
      }
  ...
  watch (file, app, type = 'defSet') {
    ...
    watcher.on('change', path => {
      ...
      logger.mark(`[茶插件][修改配置文件][${type}][${app}]`)
      if (this[`change_${app}`]) {
        this[`change_${app}`]()
      }
    })
