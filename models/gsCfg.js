...
class GsCfg {
  constructor () {
    /** 默认设置 */
    this.defSetPath = './plugins/tea-plugin/defSet/'
    this.defSet = {}

    /** 用户设置 */
    this.configPath = './plugins/tea-plugin/config/'
    this.config = {}
  ...
  cpCfg (app, name) {
    if (!fs.existsSync('./plugins/tea-plugin/config')) {
      fs.mkdirSync('./plugins/tea-plugin/config')
    }

    let set = `./plugins/tea-plugin/config/${app}.${name}.yaml`
    if (!fs.existsSync(set)) {
      fs.copyFileSync(`./plugins/tea-plugin/defSet/${app}/${name}.yaml`, set)
    }
  }
